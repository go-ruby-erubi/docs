...
`go-ruby-erubi/erubi` reimplements Ruby's `erubi` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-erubi/erubi` directly, with no Ruby runtime;
...
Because `erubi` is pure Go with cgo disabled, it also compiles to **WebAssembly**
